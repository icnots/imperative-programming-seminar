...
Egy input streamről való olvasáshoz a legbiztonságosabb az `fgets` függvény használata.
A függvény három paramétere a következőkre jó:
`char* fgets( char *str, int count, FILE *stream );`
* `str` a karaktertömb, amibe olvasunk
* `count` hány karaktert tudunk a beolvasáskor az `str` tömbünkbe írni (A `'\0'` karaktert is beleértve)
* `stream` az input streamünk.
  * amennyiben egy `f` megnyitott fájlból olvasunk, akkor az
  * amennyiben a standard inputról szeretnénk, akkor `stdin`.
* A visszatérési érték sikeres beolvasás esetén `str`, ha nem tudtunk beolvasni (befejeződött az input stream fájlvége / EOF karakter), akkor `NULL` pointerrel tér vissza.
